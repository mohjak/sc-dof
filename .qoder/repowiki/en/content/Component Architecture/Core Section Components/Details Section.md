...
## Update Summary
**Changes Made**   
- Updated the Location Card section to reflect the addition of an embedded Google Maps iframe
- Added new section for Embedded Map Integration
- Updated architecture overview diagram to include iframe component
- Enhanced troubleshooting guide with iframe-specific issues
- Added guidance for customizing map display and responsiveness
...
- An embedded Google Maps iframe for interactive venue navigation
...
DS --> IFRAME["Embedded Map<br/>Google Maps iframe"]
DS --> GRID["Responsive Grid<br/>Tailwind grid-cols-*"]
```

**Diagram sources**
- [DetailsSection.tsx](file://src/components/DetailsSection.tsx#L1-L81)
...
## Core Components
- DetailsSection: Renders three cards with icons and translated content, applies a green background theme, and overlays a decorative Syrian flag watermark. The Location card is implemented as an anchor element linking to Google Maps, and an embedded iframe provides interactive map navigation.
...
The DetailsSection composes three cards inside a responsive grid. Each card uses Lucide icons and displays translated content from the LanguageContext. The Location card opens an external Google Maps link in a new tab with safe attributes. Below the cards, an embedded Google Maps iframe provides interactive venue navigation. The section’s background uses the custom green color from the theme, and a watermark image is overlaid behind the content.
...
- Purpose: Provides interactive venue navigation through an embedded Google Maps iframe below the location card.
- Implementation: Uses an iframe element with Google Maps embed URL, configured with responsive dimensions, lazy loading, and proper security policies.
- Configuration: The iframe includes attributes for fullscreen capability, lazy loading, and referrer policy to enhance performance and security.
- Visuals: The embedded map is contained within a rounded container with border and shadow effects for visual separation from the rest of the content.

Accessibility and responsiveness:
- The iframe has a descriptive title attribute for screen readers.
- The map container uses responsive width (100%) and fixed height (350px) with overflow hidden to maintain aspect ratio.
- The embedded map automatically adapts to different screen sizes while maintaining usability.

**Section sources**
- [DetailsSection.tsx](file://src/components/DetailsSection.tsx#L61-L74)
...
- Map configuration: The embedded map URL can be updated in the DetailsSection component to reflect venue changes.
...
- Embedded map: The iframe uses lazy loading and is positioned below the fold to prioritize initial page load performance.
...
- Embedded map not loading:
  - Check that the iframe src URL is valid and accessible.
  - Verify that the referrerPolicy and loading attributes are correctly configured.
  - Ensure the container has sufficient height and is not hidden by other elements.
